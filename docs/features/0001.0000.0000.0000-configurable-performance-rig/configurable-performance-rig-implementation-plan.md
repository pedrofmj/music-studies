...
- opt-in `json-c` 0.17 Linux build, parse-cost, process-RSS, and binary-footprint
  proof with no dependency in the default CLI; and
...
Open Milestone 0 gates include a clean transient-free Airstar capture, the
repeatable startup transcript, the Windows `json-c` proof, Windows reference
build and IPC proof, and the remaining backend boundary decisions. No
