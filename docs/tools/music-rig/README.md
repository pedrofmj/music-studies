...
one stable input and eight fader-specific outputs. Its explicit Linux JACK host
coalesces repeated updates for each fader within one JACK cycle, emits the latest
value/frame only to that fader's output, and reports the discarded intermediate
updates. It contains no discovery, connect,
